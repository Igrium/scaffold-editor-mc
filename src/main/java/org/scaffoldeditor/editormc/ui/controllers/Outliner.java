...
public class Outliner {
    ...
    public void initEditor(ScaffoldEditor editor) {
        this.editor = editor;
...
//				alert.setHeaderText("Delete Group");
    ...
    public void createGroup() {
        ...
        EditorOperationManager.getInstance().runOperation(new AddGroupOperation(getSelectedGroup(), name.get(), getLevel()));
    }

    public TreeView<StackItem> getTree() {
        return entityList;
    }
    ...
    @Deprecated
    public static Outliner load(ScaffoldUI parent) {
    ...
    /**
     * Load and open the outliner.
     * @param editor Editor to attach to.
     * @return Loaded outliner.
     */
    public static Outliner open(ScaffoldEditor editor) {
        FXMLLoader loader = new FXMLLoader(Outliner.class.getResource("/assets/scaffold/ui/outliner.fxml"));
        try {
            loader.load();
        } catch (IOException e) {
            throw new RuntimeException("Unable to open outliner UI.", e);
        }

        Outliner outliner = loader.getController();
        outliner.initEditor(editor);
