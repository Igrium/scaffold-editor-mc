...
public class FakeSession extends Session {
    ...
    public static final LevelInfo EDITOR_LEVEL_INFO = new LevelInfo("", GameMode.SPECTATOR, false, Difficulty.NORMAL, true, new GameRules(), DataPackSettings.SAFE_MODE);
    
    protected DynamicRegistryManager dynRegMan;
    
    public FakeSession(LevelStorage levelStorage, DynamicRegistryManager.Immutable registry) throws IOException {
        levelStorage.super(".scaffold_placeholder");
        this.dynRegMan = registry;
    ...
    public SaveProperties readLevelProperties(DynamicOps<NbtElement> ops, DataPackSettings dataPackSettings,
            Lifecycle lifecycle) {
        return generateSaveProperties(ops, dataPackSettings, lifecycle);
    }
    ...
    public LevelProperties generateSaveProperties(DynamicOps<NbtElement> ops, DataPackSettings dataPackSettings, Lifecycle lifecycle) {
        LevelInfo levelInfo = EDITOR_LEVEL_INFO;
        GeneratorOptions generatorOptions = WorldPresets.createDefaultOptions(dynRegMan);
    ...
    //     CompletableFuture<DataPackContents> completableFuture = DataPackContents.reload(
    ...
    //     DataPackContents serverResourceManager = (DataPackContents) completableFuture.get();
